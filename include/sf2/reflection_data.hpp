...
namespace sf2 {
	...
	struct String_literal {
		...
		constexpr String_literal(const char (&data)[N])
		  : data(data), len(N - 1), hash(details::calc_hash(data, len))
		{
		}
		String_literal(const std::string& str)
...
namespace std {
	...
	struct hash<sf2::String_literal> {
		size_t operator()(const sf2::String_literal& s) const { return s.hash; }
	};
} // namespace std
...
namespace sf2 {
	...
	class Enum_info {
	  ...
	  public:
		Enum_info(String_literal name, std::initializer_list<Value_type> v) : _name(name)
		{
			for(auto& e : v) {
	  ...
	  private:
		struct Enum_hash {
			auto operator()(T v) const { return static_cast<std::size_t>(v); }
		};
	...
	class Struct_info {
	  public:
		...
		constexpr Struct_info(String_literal name, Member_data<T, MemberT>... members)
		  : _name(name), _member_names{{std::get<1>(members)...}}, _members(std::get<0>(members)...)
		{
		}
		...
		inline typename std::enable_if<I == member_count, void>::type for_each(FuncT) const
		{
		}

		template <std::size_t I = 0, typename FuncT>
		        inline typename std::enable_if < I<member_count, void>::type for_each(FuncT f) const
		{
	...
	struct is_annotated_struct {
	  private:
		...
		template <typename C>
		static one test(std::remove_reference_t<decltype(sf2_struct_info_factory(static_cast<C*>(nullptr)))>*);
		template <typename C>
		static two test(...);
	...
	struct is_annotated_enum {
	  private:
		...
		template <typename C>
		static one test(std::remove_reference_t<decltype(sf2_enum_info_factory(static_cast<C*>(nullptr)))>*);
		template <typename C>
		static two test(...);
	...
	template <class T>
	struct is_annotated {
		enum { value = is_annotated_struct<T>::value || is_annotated_enum<T>::value };
	};
